...
const deploySave = async (sender, addresses, mBTC, bAssets, deployVault = true) => {
    ...
    const unwrapperFactory = await new generated_1.Unwrapper__factory(sender);
    const unwrapperContract = await unwrapperFactory.deploy(addresses.nexus);
    const sImpl = await new generated_1.SavingsContract__factory(sender).deploy(addresses.nexus, mBTC.address, unwrapperContract.address);
    ...
    // Vault impl
    if (deployVault) {
        const vImpl = await new generated_1.BoostedVault__factory(sender).deploy(addresses.nexus, savingContract.address, addresses.boostDirector, math_1.simpleToExactAmount(3000, 18), COEFF, addresses.zeno);
        ...
        const savingsVault = await new generated_1.BoostedVault__factory(sender).attach(vProxy.address);
        console.log(`Deployed Vault Proxy to ${vProxy.address}. gas used ${receiptVaultProxy.gasUsed}`);
        // SaveWrapper
        const wrapper = await new generated_1.SaveWrapper__factory(sender).deploy(addresses.nexus);
...
config_1.task("reDeployMBTC", "Re-deploys the mBTC contracts given bAsset addresses").setAction(async (_, hre) => {
    ...
    const bAssets = await Promise.all(bAssetsRaw.map(async (b) => ({
        ...
        integrator: b.integrator,
        txFee: b.txFee,
        symbol: await erc20Factory.attach(b.address).symbol(),
    })));
    // 2. Deploy mBTC
    const mBTC = await deployMasset(deployer, addresses, ethers, bAssets);
    // 3. Mint initial supply
